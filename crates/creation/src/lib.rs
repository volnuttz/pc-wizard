...
#[allow(clippy::too_many_lines)]
fn collect_origin() -> Result<OriginDraft> {
    ...
    for list in magic_lists {
        origin
            ...
            .push(collect_magic_initiate(&list)?);
    }
...
fn collect_magic_initiate(list: &str) -> Result<MagicInitiateChoice> {
    ...
    Ok(MagicInitiateChoice {
        ...
        cantrips: choose_pair("Magic Initiate cantrips", rules.cantrips)?,
        level_one_spell: choose("Magic Initiate level 1 spell", rules.level_one_spells)?,
    })
}

fn collect_abilities(origin: &OriginDraft) -> Result<AbilityScores> {
    ...
    let (method, scores) = match method_label.as_str() {
        ...
        "Assign the standard array" => (
            ...
            assign_score_pool(pc_wizard_srd_data::STANDARD_ARRAY)?,
        ),
        "Roll 4d6 and drop the lowest" => {
            ...
            (AbilityGenerationMethod::Random, assign_score_pool(values)?)
        }
        _ => (
            AbilityGenerationMethod::PointBuy,
            collect_point_buy_scores()?,
        ),
    };
    ...
    .validate()?;
    apply_background_increases(&scores, &origin.background)
}
...
fn assign_score_pool(mut pool: [u8; 6]) -> Result<AbilityScores> {
    let mut values = [0; 6];
...
fn collect_point_buy_scores() -> Result<AbilityScores> {
...
fn apply_background_increases(scores: &AbilityScores, background: &str) -> Result<AbilityScores> {
...
#[allow(clippy::too_many_lines)]
fn collect_build(origin: &OriginDraft) -> Result<BuildDraft> {
...
fn collect_details() -> Result<DetailsDraft> {
    Ok(DetailsDraft {
        backstory: optional_prompt("Backstory")?,
        appearance: optional_prompt("Appearance")?,
        personality: optional_prompt("Personality")?,
    })
}
