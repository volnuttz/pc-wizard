...
mod tests {
    ...
    const COMPLETE_ROGUE: &str = include_str!("../../../contracts/fixtures/complete-rogue-v1.json");

    #[derive(serde::Deserialize)]
    struct ClassParityCase {
        character: serde_json::Value,
        derived: serde_json::Value,
        class_resources: Vec<super::ClassResource>,
    }

    #[derive(serde::Deserialize)]
    struct OriginParityContract {
        backgrounds: Vec<DerivedParityCase>,
        species: Vec<DerivedParityCase>,
    }

    #[derive(serde::Deserialize)]
    struct DerivedParityCase {
        character: serde_json::Value,
        derived: serde_json::Value,
    }

    fn derived_json(character: &Character) -> serde_json::Value {
        ...
        serde_json::json!({
            "proficiency_bonus": character.proficiency_bonus(), "hit_points": character.hit_points(),
            "armor_class": character.armor_class(), "initiative_modifier": character.initiative_modifier(),
            "speed": character.speed(), "passive_perception": character.passive_perception(),
            "skills": character.skills().into_iter().collect::<Vec<_>>(), "languages": languages,
            "equipment": character.inventory(), "weapon_attacks": character.weapon_attacks(),
            "spellcasting_ability": character.spellcasting_ability(), "spell_save_dc": character.spell_save_dc(),
            "spell_attack_bonus": character.spell_attack_bonus(), "spell_slots": character.spell_slots(),
            "spellcasting_profiles": character.spellcasting_profiles(), "spells": character.spell_table_entries(),
        })
    }
    ...
    #[test]
    #[allow(clippy::too_many_lines)]
    fn every_class_matches_the_python_derived_contract() {
        let cases: Vec<ClassParityCase> = serde_json::from_str(include_str!(
            "../../../contracts/fixtures/class-parity-v1.json"
        ))
        .expect("class parity fixture");
        for case in cases {
            let character = Character::from_json(&case.character.to_string())
                .expect("Rust accepts Python class fixture");
            let derived = case.derived;
            let class_name = character.character_class.clone();
            assert_eq!(
                serde_json::json!(character.proficiency_bonus()),
                derived["proficiency_bonus"],
                "{class_name} proficiency"
            );
            assert_eq!(
                serde_json::json!(character.hit_points()),
                derived["hit_points"],
                "{class_name} HP"
            );
            assert_eq!(
                serde_json::json!(character.armor_class()),
                derived["armor_class"],
                "{class_name} AC"
            );
            assert_eq!(
                serde_json::json!(character.initiative_modifier()),
                derived["initiative_modifier"],
                "{class_name} initiative"
            );
            assert_eq!(
                serde_json::json!(character.speed()),
                derived["speed"],
                "{class_name} speed"
            );
            assert_eq!(
                serde_json::json!(character.passive_perception()),
                derived["passive_perception"],
                "{class_name} passive perception"
            );
            assert_eq!(
                serde_json::to_value(character.skills().into_iter().collect::<Vec<_>>())
                    .expect("skills"),
                derived["skills"],
                "{class_name} skills"
            );
            ...
            languages.extend(character.class_choices.additional_language.iter().cloned());
            assert_eq!(
                serde_json::to_value(languages).expect("languages"),
                derived["languages"],
                "{class_name} languages"
            );
            assert_eq!(
                serde_json::to_value(character.inventory()).expect("equipment"),
                derived["equipment"],
                "{class_name} equipment"
            );
            assert_eq!(
                serde_json::to_value(character.weapon_attacks()).expect("attacks"),
                derived["weapon_attacks"],
                "{class_name} attacks"
            );
            assert_eq!(
                serde_json::json!(character.spellcasting_ability()),
                derived["spellcasting_ability"],
                "{class_name} spell ability"
            );
            assert_eq!(
                serde_json::json!(character.spell_save_dc()),
                derived["spell_save_dc"],
                "{class_name} spell DC"
            );
            assert_eq!(
                serde_json::json!(character.spell_attack_bonus()),
                derived["spell_attack_bonus"],
                "{class_name} spell attack"
            );
            assert_eq!(
                serde_json::to_value(character.spell_slots()).expect("slots"),
                derived["spell_slots"],
                "{class_name} slots"
            );
            assert_eq!(
                serde_json::to_value(character.spellcasting_profiles()).expect("profiles"),
                derived["spellcasting_profiles"],
                "{class_name} profiles"
            );
            assert_eq!(
                serde_json::to_value(character.spell_table_entries()).expect("spells"),
                derived["spells"],
                "{class_name} spells"
            );
            assert_eq!(
    ...
    #[test]
    fn every_background_and_species_matches_the_python_derived_contract() {
        let contract: OriginParityContract = serde_json::from_str(include_str!(
            "../../../contracts/fixtures/origin-parity-v1.json"
        ))
        .expect("origin parity fixture");
        for case in contract.backgrounds.into_iter().chain(contract.species) {
            let character = Character::from_json(&case.character.to_string())
                .expect("Rust accepts Python origin fixture");
            assert_eq!(
                derived_json(&character),
                case.derived,
                "{} / {}",
