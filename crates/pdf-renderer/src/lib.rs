//! Supported-template validation and `AcroForm` rendering.
...
pub fn render_character(
    ...
    let mut values = BTreeMap::from([
        ...
        ("Text6".to_owned(), character.background.clone()),
        ("Text7".to_owned(), character.character_class.clone()),
        ("Text8".to_owned(), character.species.clone()),
        ...
        (
            ...
            format!(
                "1d{}",
                pc_wizard_srd_data::class_rule(&character.character_class)
                    .map_or(0, |rule| rule.hit_die)
            ),
        ),
        (
            "Text19".to_owned(),
    ...
    for (ability, score_field, modifier_field, save_field, checkbox) in [
        ...
        let proficient = pc_wizard_srd_data::class_rule(&character.character_class)
            .is_some_and(|rule| rule.saves.contains(&ability));
        let save = character.abilities.modifier(ability)
            + if proficient {
                i16::from(character.proficiency_bonus())
            } else {
                0
            };
        values.insert(save_field.to_owned(), signed(save));
        values.insert(
            checkbox.to_owned(),
            if proficient { "/Yes" } else { "/Off" }.to_owned(),
        );
    }
...
fn project_spell_rows(character: &Character, values: &mut BTreeMap<String, String>) {
    const NOTES: [&str; 30] = [
